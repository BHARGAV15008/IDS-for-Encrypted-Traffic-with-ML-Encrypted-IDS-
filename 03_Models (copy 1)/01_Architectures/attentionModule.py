...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        assert kLen == vLen, f"key and value must have same sequence length, got key_len={kLen}, value_len={vLen}"
        
        residual = query
        
        # Linear projections and reshape
        q = self.qLinear(query).view(batch, qLen, self.numHeads, self.headDim).transpose(1, 2)
        k = self.kLinear(key).view(batch, kLen, self.numHeads, self.headDim).transpose(1, 2)
        v = self.vLinear(value).view(batch, vLen, self.numHeads, self.headDim).transpose(1, 2)
        
        # Apply attention
        output, attention = self.attention(q, k, v, mask)
        
        # Concatenate heads and apply output projection
        output = output.transpose(1, 2).contiguous().view(batch, qLen, self.embedDim)
...
class DualAttention(nn.Module):
    ...
    def __init__(self, numFeatures: int, hiddenDim: int):
        ...
        self.temporalAttention = TemporalAttention(numFeatures)
...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        assert outputDim % numHeads == 0, f"outputDim ({outputDim}) must be divisible by numHeads ({numHeads})"
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        """
        # Self-attention with residual (already handled inside MultiHeadAttention)
