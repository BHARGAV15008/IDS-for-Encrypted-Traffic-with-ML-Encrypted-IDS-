...
class TemporalConvolutionalNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(numLayers):
            ...
            # Causal convolution (padding on left only)
            layers.append(nn.Conv1d(
                currentChannels,
                hiddenChannels,
                kernel_size=kernelSize,
            ...
            layers.append(nn.BatchNorm1d(hiddenChannels))
            layers.append(nn.ReLU(inplace=True))
            layers.append(nn.Dropout(dropoutRate))
            
            # Residual connection
            if currentChannels != hiddenChannels:
                self.residual = nn.Conv1d(currentChannels, hiddenChannels, kernel_size=1)
            else:
                self.residual = None
                
            currentChannels = hiddenChannels
            
        self.tcnLayers = nn.Sequential(*layers)
